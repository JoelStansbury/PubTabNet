import pandas as pd
import numpy as np
import pytesseract as tess
from sklearn.mixture import GaussianMixture as GMM

#  ------------ verbose utils --------------
import cv2
import matplotlib.pyplot as plt
COLORS = {
    'black':(0,0,0),
    'red':(150,0,0),
    'green':(0,150,0),
    'blue':(0,0,150)
}

def draw(im, bbox, color='black'):
    im = np.array(im)  # in case it is in PIL format
    x,y,w,h = bbox
    
    if isinstance(color, str):
        color = COLORS[color]
    
    pt1 = (x, y)
    pt2 = (x+w, y+h)
    return cv2.rectangle(im, pt1=pt1, pt2=pt2,
        color=color, thickness=1
    )

# ------------ tess utils --------------
def tessdata_to_df(tessdata, keep_garbage=False):
    """Ingests a string repr of tesseract output and spits out a dataframe"""
    rows = [r.split("\t") for r in tessdata.split("\n")[:-1]]
    h = rows[0]
    rows = rows[1:]
        
    df = pd.DataFrame(rows)
    df.columns = h
    
    # set types
    dtypes = [int]*10 + [float, str]
    for c,t in zip(df.columns, dtypes):
        df[c] = df[c].values.astype(t)
    
    if not keep_garbage:
        df = df[[x.strip() != "" for x in df["text"]]]
        df = df[df["conf"] > 0].reset_index()
    
    return df


def fit_bboxes_to_text(im, tessdata):
    """
    Tesseract bboxes sometimes have very wide margins on them. This is bad for the purpose
    of determining the grid that defines the layout.
    Shrinks each side of the bbox until ~8% of the darkness is lost.
    
    im <PIL.Image or np.array()>: image used for cropping out the boxes defined in tessdata
    
    tessdata <Pandas.DataFrame>: Should be a dataframe with columns ["left","top","width","height"]
        as is returned by tessdata_to_df
    """
    df_left = tessdata["left"].values
    df_top = tessdata["top"].values
    df_width = tessdata["width"].values
    df_height = tessdata["height"].values
    
    im = np.array(im).sum(axis=2)
    im = im / im.max()
    im = abs(im - 1)
    for i, bbox in enumerate(tessdata[['left', 'top', 'width', 'height']].values):
        x,y,w,h = bbox
        cropped = im[y:y+h, x:x+w]
        
        v_sum = cropped.sum(axis=1)
        h_sum = cropped.sum(axis=0)
        
        top = 0
        while sum(v_sum[top:]) > sum(v_sum)*0.98 and top < len(v_sum)-3:
            top += 1
            
        bottom = len(v_sum)
        while sum(v_sum[top:bottom]) > sum(v_sum)*0.96 and bottom > top+2:
            bottom -= 1
            
            
        left = 0
        while sum(h_sum[left:]) > sum(h_sum)*0.98 and left < len(h_sum)-3:
            left += 1
            
        right = len(h_sum)
        while sum(h_sum[left:right]) > sum(h_sum)*0.96 and right > left+2:
            right -= 1
        
        df_left[i] = x + left
        df_top[i] = y + top
        df_width[i] = right-left
        df_height[i] = bottom-top
        
    df = tessdata.copy()
    df["left"] = df_left
    df["top"] = df_top
    df["width"] = df_width
    df["height"] = df_height
    return df


def scale(im, scale):
    return im.resize((int(x*scale) for x in im.size))


def im_to_data(im, scaling_factor=1):
    """
    im: Image
    scaling_factor: Amount to scale the image (evenly along both axes)
        This is solely for the purpose of improving OCR results. The
        image is scaled before passing to Tesseract, then the output
        of Tesseract is scaled down to fit the dimensions of the original
        image. 
        Default = 1 (no scaling)
    """
    tessdata = tess.image_to_data(scale(im, scaling_factor), config="--psm 1")

    df = tessdata_to_df(tessdata)
    df["left"] = (df["left"] // scaling_factor).astype(int)
    df["top"] = (df["top"] // scaling_factor).astype(int)
    df["width"] = (df["width"] // scaling_factor).astype(int)
    df["height"] = (df["height"] // scaling_factor).astype(int)
    return df


# ------------ bbox utils --------------
def partition(boxes, axis=0, margin=0):
    """
    partitions a list of bounding boxes (in x,y,w,h format)
    into a list of lists, where each element of the inner
    lists is at least `margin` units away from the bboxes
    from every other group.
    """
    boxes.sort(key=lambda x: x[axis]+x[axis+2]) # sort by x_max
    boxes.sort(key=lambda x: x[axis]) # sort by x_min
    
    groups = []
    
    last_x = boxes[0][axis] + boxes[0][axis+2]
    last_i = 0
    
    for i,b in enumerate(boxes):
        if b[axis] > last_x+margin:
            groups.append(boxes[last_i:i])
            last_x = b[axis]+b[axis+2]
            last_i = i
            
        elif b[axis]+b[axis+2]>last_x:
            last_x = b[axis]+b[axis+2]
    
    groups.append(boxes[last_i:])
    return groups


def fit_contents(boxes):
    """
    returns the smallest box which encompases all of the boxes
    """
    x = min([b[0] for b in boxes])
    y = min([b[1] for b in boxes])
    w = max([b[0]+b[2] for b in boxes]) - x
    h = max([b[1]+b[3] for b in boxes]) - y
    return x,y,w,h


def contains(coords1, coords2):
    xa, ya, wa, ha = coords1
    xb, yb, wb, hb = coords2
    px = xb + wb/2
    py = yb + hb/2
    return xa < px and (xa + wa) > px and ya < py and (ya + ha) > py


# ------------ parsing --------------
def group_ocr_bboxes(ocr_df, margin_x_factor, margin_y_factor):
    """
    Returns bboxes of cells according to the minimum margins
    provided in the args

    ocr_df: DataFrame as returned by im_to_data with at least the following columns
        'left', 'top', 'width', 'height', 'text'

    margin_x_factor: <float> This value is used to determine the distance at which two
        tokens within ocr_df will be joined into a one cell (i.e. multi-word datum).
        Specifically, this value is multiplied by the average token character width
        to determine the threshold in terms of number of pixels.
    
    margin_y_factor: <float> Same as margin_x_factor, but pertains to multi-line cells.
    """
    boxes = list(ocr_df[['left', 'top', 'width', 'height']].values)
    
    heights = [b[2] for b in boxes]
    avg_height = sum(heights)/len(heights)
    
    # normalized_widths = [b[3]/len(ocr_df["text"][i]) for i, b in enumerate(boxes)]
    # avg_width = sum(normalized_widths)/len(normalized_widths)
    
    margin_y = avg_height * margin_y_factor
    margin_x = avg_height * margin_x_factor # * avg_width
    
    rows = []
    for row in partition(boxes, 1, margin_y):
        groups = partition(row, 0, margin_x)
        r = []
        for g in groups:
            r.append(fit_contents(g))
        rows.append(r)
    
    return rows


def assign_data_to_cells(ocr_df, rows):
    """
    Populates the cells returned from group_ocr_bboxes with
    the contents returned from tesseract.
    Uses a GaussianMixture model do assign column positions.
    """
    new_df = []
    texts = ocr_df["text"]
    boxes = list(ocr_df[['left', 'top', 'width', 'height']].values)
    
    n_cols = 0
    for i, row in enumerate(rows):
        if len(row) > n_cols:
            n_cols = len(row)
        for j, cell in enumerate(row):
            x,y,w,h = cell
            text = " ".join([texts[i] for i,b in enumerate(boxes) if contains(cell, b)])
            new_df.append(
                {
                    "left": x,
                    "top": y,
                    "width": w,
                    "height": h,
                    "text": text,
                    "row": i,
                    "backup_col":j,
                }
            )
    
    # Assign column positions
    g_model = GMM(n_components=n_cols)
    new_df = pd.DataFrame(new_df)
    boxes = list(new_df[['left', 'top', 'width', 'height']].values)
    X = [
        [x[0]] #, x[0]+x[2]] 
        for x in boxes
    ]
    y = g_model.fit_predict(X)
    
    # sort categories by x_pos
    cats = list(
        zip(*sorted(zip(g_model.means_.min(axis=1), np.arange(n_cols))))
    )[1]
    
    # assign column positions to new df
    column = []
    for c in y:
        column.append(cats.index(c))
    
    new_df["col"] = column
    
    return new_df


def metadata_to_data(df):
    rows = max(df["row"])
    cols = max(df["col"])
    data = [["" for i in range(cols+1)] for j in range(rows+1)]
    for cell in df.to_dict("records"):
        if data[cell["row"]][cell["col"]]:
            data[cell["row"]][cell["backup_col"]] = cell["text"]
        else:
            data[cell["row"]][cell["col"]] = cell["text"]
    return pd.DataFrame(data)


def im_to_table(
    im, 
    scaling_factor=1, 
    margin_x_factor=4, 
    margin_y_factor=0.1,
    verbose=False,
):
    
    ocr_df = im_to_data(im, scaling_factor)
    rows = group_ocr_bboxes(ocr_df, margin_x_factor, margin_y_factor)
    df = assign_data_to_cells(ocr_df, rows)

    if verbose:
        im_cp = np.array(im)
        
        boxes = list(ocr_df[['left', 'top', 'width', 'height']].values)
        for box in boxes:
            im_cp = draw(im_cp, box, "blue")

        boxes = list(df[['left', 'top', 'width', 'height']].values)
        for box in boxes:
            im_cp = draw(im_cp, box, "red")
            
        plt.imshow(im_cp)
        plt.show()
    
    return metadata_to_data(df)